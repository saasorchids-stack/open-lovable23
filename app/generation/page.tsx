...
Focus on the key sections and content, making it clean and modern.`;
            ...
            if (line.startsWith('data: ')) {
              try {
                ...
                if (data.type === 'start') {
                  ...
                  // Accumulate streamed code as fallback in case 'complete' never arrives
                  streamedCodeAccumulator += data.text;
                  setGenerationProgress(prev => {
                    const newStreamedCode = prev.streamedCode + data.text;
                  ...
                  generatedCode = data.generatedCode;
                  explanation = data.explanation;
                  console.log('[clone] Received complete event, generatedCode length:', generatedCode?.length || 0);
        ...
        // If 'complete' event never arrived (e.g. Vercel timeout), use accumulated stream data
        if (!generatedCode && streamedCodeAccumulator) {
          console.warn('[clone] complete event missing, using accumulated stream data as fallback, length:', streamedCodeAccumulator.length);
          // Check if the streamed data contains usable file tags
          if (streamedCodeAccumulator.includes('<file path=')) {
            generatedCode = streamedCodeAccumulator;
          }
        }
...
Make it responsive and modern.`;
        ...
        if (generatedCode) {
          ...
          // Pass createdSandbox as override to avoid stale closure issue where
          // sandboxData is still null because setSandboxData hasn't re-rendered yet
